...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-full pb-4 space-y-6">
      <div className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${getScoreGradient(score)} p-6 border border-border/40`}>
        <div className="absolute top-0 right-0 w-40 h-40 bg-gradient-to-bl from-primary/8 to-transparent rounded-full -translate-y-1/3 translate-x-1/3" />
        <div className="absolute bottom-0 left-0 w-28 h-28 bg-gradient-to-tr from-primary/5 to-transparent rounded-full translate-y-1/3 -translate-x-1/3" />
        ...
        <div className="relative flex items-center gap-6">
          <div className="relative flex-shrink-0">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className={`text-3xl font-bold tracking-tight ${getScoreColor(score)}`} data-testid="text-legal-score">{score}</span>
              <span className="text-[9px] text-muted-foreground uppercase tracking-widest font-medium">Score</span>
            </div>
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <h2 className="text-lg font-bold mb-1.5">Legal Safety Score</h2>
      ...
      <div className="grid grid-cols-3 gap-3">
        <Card className="border-border/40" data-testid="card-contracts-count">
          <CardContent className="p-4 text-center">
            <div className="h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center mx-auto mb-2.5">
              <FileText className="h-5 w-5 text-primary" />
            </div>
            <div className="text-2xl font-bold tracking-tight" data-testid="text-contracts-count">{data?.contractsCount ?? 0}</div>
            <div className="text-[11px] text-muted-foreground font-medium mt-0.5">Contracts</div>
          </CardContent>
        </Card>

        <Card className="border-border/40" data-testid="card-alerts-count">
          <CardContent className="p-4 text-center">
            <div className="h-10 w-10 rounded-xl bg-destructive/10 flex items-center justify-center mx-auto mb-2.5 relative">
            ...
            <div className="text-2xl font-bold tracking-tight" data-testid="text-alerts-count">{totalAlerts}</div>
            <div className="text-[11px] text-muted-foreground font-medium mt-0.5">Tracked</div>
          </CardContent>
        </Card>

        <Card className="border-border/40" data-testid="card-scans-count">
          <CardContent className="p-4 text-center">
            <div className="h-10 w-10 rounded-xl bg-secondary flex items-center justify-center mx-auto mb-2.5">
              <Camera className="h-5 w-5 text-secondary-foreground" />
            </div>
            <div className="text-2xl font-bold tracking-tight" data-testid="text-scans-count">{data?.recentScans?.length ?? 0}</div>
            <div className="text-[11px] text-muted-foreground font-medium mt-0.5">Scans</div>
          </CardContent>
        </Card>
      </div>

      {alerts.urgent.length > 0 && (
        <div data-testid="section-urgent-alerts">
      ...
      {alerts.dueSoon.length > 0 && (
        <div data-testid="section-due-soon-alerts">
          <div className="flex items-center gap-2 mb-3 px-1">
      ...
      {alerts.safe.length > 0 && (
        <div data-testid="section-safe-alerts">
          <div className="flex items-center gap-2 mb-3 px-1">
      ...
      {totalAlerts === 0 && (
        <Card className="border-dashed border-border/40" data-testid="card-no-alerts">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Link href="/quick-scan">
            <Card className="hover-elevate cursor-pointer h-full border-border/40 transition-all duration-200" data-testid="action-screenshot-intelligence">
              <CardContent className="p-4 flex flex-col items-center text-center">
                <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-primary to-primary/80 flex items-center justify-center mb-3 shadow-sm">
                  <Camera className="h-6 w-6 text-primary-foreground" />
                </div>
                <span className="font-semibold text-sm">Screenshot Intel</span>
                <span className="text-[11px] text-muted-foreground mt-0.5">Snap & analyze</span>
              </CardContent>
            </Card>
          </Link>

          <Link href="/negotiation-coach">
            <Card className="hover-elevate cursor-pointer h-full border-border/40 transition-all duration-200" data-testid="action-advocate-chat">
              <CardContent className="p-4 flex flex-col items-center text-center">
                <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-primary/90 to-primary/60 flex items-center justify-center mb-3 shadow-sm">
                  <MessageSquare className="h-6 w-6 text-primary-foreground" />
                </div>
                <span className="font-semibold text-sm">Advocate Chat</span>
                <span className="text-[11px] text-muted-foreground mt-0.5">Ask anything</span>
              </CardContent>
            </Card>
          </Link>

          <Link href="/signed-contracts">
            <Card className="hover-elevate cursor-pointer h-full border-border/40 transition-all duration-200" data-testid="action-obligations">
              <CardContent className="p-4 flex flex-col items-center text-center">
                <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-secondary to-secondary/80 flex items-center justify-center mb-3">
                  <FileText className="h-6 w-6 text-secondary-foreground" />
                </div>
                <span className="font-semibold text-sm">Obligations</span>
                <span className="text-[11px] text-muted-foreground mt-0.5">Track deadlines</span>
              </CardContent>
            </Card>
          </Link>

          <Link href="/emergency">
            <Card className="hover-elevate cursor-pointer h-full border-border/40 transition-all duration-200" data-testid="action-emergency">
              <CardContent className="p-4 flex flex-col items-center text-center">
                <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-destructive to-destructive/80 flex items-center justify-center mb-3 shadow-sm">
                  <Siren className="h-6 w-6 text-destructive-foreground" />
                </div>
                <span className="font-semibold text-sm">Emergency</span>
                <span className="text-[11px] text-muted-foreground mt-0.5">Get help fast</span>
              </CardContent>
            </Card>
          </Link>
        </div>
      ...
      {data?.recentScans && data.recentScans.length > 0 && (
        <div data-testid="section-recent-scans">
      ...
      {data?.recentChats && data.recentChats.length > 0 && (
        <div data-testid="section-recent-chats">
          <div className="flex items-center justify-between gap-4 mb-3 px-1">
